...
def _client(transport) -> AnthropicClient:
	return AnthropicClient(
		api_key="k", model="claude-sonnet-5", transport=transport, retry=RetryPolicy(sleep=lambda s: None)
	)


def test_structured_forces_one_tool_and_reads_its_input():
	transport = FakeTransport(
		[_message(tool_uses=[{"id": "tu1", "name": "thing", "input": {"a": 7}}], stop_reason="tool_use")]
	)
	...
	assert (
		result.data == {"a": 7}
		and result.tokens_in == 200
		and result.tokens_out == 40
		and result.raw_id == "msg_1"
	)
...
def test_with_tools_loop_replays_transcript_with_tool_results():
	transport = FakeTransport(
		[
			_message(
				text="let me check",
				tool_uses=[{"id": "tu1", "name": "lookup", "input": {"q": "x"}}],
	...
	assert first["tools"] == [{"name": "lookup", "description": "d", "strict": True, "input_schema": SCHEMA}]
		...
		"content": [
			{
				"type": "tool_result",
				"tool_use_id": "tu1",
				"content": json.dumps({"count": 3}, ensure_ascii=False),
			}
		],
	}
