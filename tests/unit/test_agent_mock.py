...
def test_keyed_fixture_wins_over_default(tmp_path):
	...
	(tmp_path / "extract" / "default.json").write_text(json.dumps({"data": {"a": "default"}}), encoding="utf-8")
	...
	other = client.structured(purpose="extract", system="s", user=[TextPart("other")], schema=SCHEMA, schema_name="t")
...
def test_scripted_answers_take_precedence_and_calls_are_recorded(tmp_path):
	...
	assert client.structured(purpose="classify", system="s", user=user, schema=SCHEMA, schema_name="t").data == {"a": 2}
	assert client.structured(purpose="classify", system="s", user=[TextPart("y")], schema=SCHEMA, schema_name="t").data == {"a": 1}
...
def test_with_tools_runs_scripted_calls_through_handler(tmp_path):
	client = MockLlmClient(fixtures_dir=tmp_path)
	client.add("question", {"text": "3 гүйлгээ", "tool_calls": [{"name": "count", "arguments": {"k": 1}}, {"name": "nope", "arguments": {}}]})
	seen = []
	...
	assert result.text == "3 гүйлгээ" and result.tool_calls[1].is_error and client.calls[0].tools == ("count",)
