...
class GLEntry(Document):
	...
	def validate_account_details(self) -> None:
		...
		account = frappe.get_cached_value(
			"Account", self.account, fieldname=["is_group", "docstatus", "company", "disabled"], as_dict=True
		)
		if not account:
			raise ValidationError(
				f"{self.voucher_type} {self.voucher_no}: Account {self.account} does not exist"
			)
		...
		if account.docstatus == 2:
			raise ValidationError(
				f"{self.voucher_type} {self.voucher_no}: Account {self.account} is inactive"
			)
		...
		if account.disabled:
			raise ValidationError(
				f"{self.voucher_type} {self.voucher_no}: Account {self.account} is disabled"
			)

	def validate_amounts(self) -> None:
		if flt(self.debit) and flt(self.credit):
			raise ValidationError(
				f"{self.voucher_type} {self.voucher_no}: a GL Entry row cannot carry both debit and credit"
			)
