...
runs on the ``long`` queue. An unrecognised layout is not an error: the summary carries
``unknown_layout = True``, the first rows (``preview_rows``) and the generic ``guess``
(matching/bank_import.py). The accountant is then asked, one column at a time, which role
each column plays, and the answer is saved as a ``Nyabo Bank Layout`` with ``verified = 0``
so nothing is imported on a guessed mapping (D-008). Everything the importer refuses - no
configured bank account, no lines, an unreadable file - arrives as ``BankImportError``
carrying the Mongolian text to show.
"""
...
def run_import(document_name: str, chat_id: int | str) -> dict[str, Any]:
	...
	# The unmatched lines already got their own card from matching.match.run.
